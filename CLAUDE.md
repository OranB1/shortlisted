...
## Pivot: marketplace-first, portfolio scoring demoted (2026-08-17)

**Why**: A call with Jemma Kelly (Chelsea & Westminster QI team) confirmed the real pain point is projects starting and stalling before completion (national stats: 57.9% of F1s never finish an audit they start), not a lack of portfolio-scoring tools — see the full transcript and research notes in this session's conversation history if the reasoning needs revisiting. User decided to simplify the site around the opportunity marketplace (finding/posting QIPs, audits, research, presentations) so it can be shown to Jemma's colleagues within a day or two, and build out the cofounder's structured-matching concept doc (`NHS_QI_Research_Marketplace_Concept.docx.pdf`) rather than the original portfolio-scoring-first pitch.

**IA change** (user's explicit call, via AskUserQuestion): portfolio scoring, `/imt-likelihood`, `/plan`, and `/specialties` are **demoted, not deleted** — every route still works at its direct URL, but none of them are in `NavBar.tsx` anymore (`src/components/NavBar.tsx` now just links Opportunities/About/Contact + a "Post an opportunity" button) or on the homepage. `Footer.tsx` keeps them reachable under a secondary "More tools" column so they're not orphaned. `src/app/page.tsx` was fully rewritten around the marketplace: hero pitches opportunity-matching (not portfolio scoring), the score-ring mockup was replaced with an illustrative opportunity-match card, and a stats/how-it-works section was added using the real completion-collapse and awareness-barrier numbers from the research (SMART survey, BMA survey, UK junior doctors audit study — sources named inline, not hyperlinked, since they weren't fetched as live URLs this session).

**Structured matching added** (`src/lib/data/marketplace-options.ts`, `src/lib/marketplace.ts`), following the concept doc's "same vocabulary on both sides" principle rather than the old free-text-comma-list required_skills field:
- New shared vocabulary: `SKILL_OPTIONS` (8 fixed skills — Excel, statistics, literature review, data collection/EPR, patient recruitment, poster/presentation, manuscript writing, governance training), `QI_EXPERIENCE_OPTIONS` (none/assisted/led), `RESEARCH_EXPERIENCE_OPTIONS` (none/data_collection/analysis/manuscript), `AVAILABILITY_OPTIONS` (<2h/2-5h/5-10h), each with an ordinal `_RANK` map so e.g. "led" satisfies a requirement of "assisted".
- New Supabase columns (migration `add_structured_matching_fields`, applied 2026-08-17): `profiles.qi_experience`, `profiles.research_experience`, `profiles.skills text[]`, `profiles.availability_hours`; `opportunities.preferred_skills text[]` (required_skills already existed, now populated from the same `SKILL_OPTIONS` enum instead of freeform text). `database.types.ts` regenerated to match.
- `computeMatch()` (renamed from `computeMatches`, src/lib/marketplace.ts) replaced the old boolean-checklist matcher with the concept doc's explainable weighted score: specialty interest 15%, skills 30% (required skills weighted 2x preferred within that budget), experience 20%, availability 20%, location 15% — renormalized over whichever criteria the specific opportunity actually specifies, so a sparse posting isn't penalised for fields it never asked about. Returns `{ percent, checks[] }`, used for the match-% badge on `/marketplace` cards and the ranked applicant dashboard in `/marketplace/mine`.
- Onboarding wizard (`OnboardingWizard.tsx`) gained 4 new steps (qi_experience, research_experience, skills chips, availability_hours) shared by both applicant roles (medical_student, doctor_applicant — F1/F2s do QIPs too, not just students). The old "intent" step (check_likelihood/score_portfolio/explore) was removed along with its routing — every applicant now finishes onboarding straight into `/marketplace`. The consultant_registrar completion screen's copy was also fixed — it previously said the marketplace-posting feature "isn't live yet" (stale; it's been live since before this session), now routes straight to `/marketplace/post`.
- `PostForm.tsx` and `MarketplaceBrowser.tsx` and `MyPostings.tsx` (poster's applicant dashboard) all updated to read/write the new fields and display `computeMatch()`'s output — `MyPostings.tsx` specifically now sorts applicants by match % descending, matching the concept doc's "ranked applicant view" (Student | Match | Year | QI | Skills/availability).

**Deliberately not built this session** (flagged to the user as later-phase, per AskUserQuestion): real AI-driven shortlisting (the concept doc's own "Recommended MVP" explicitly says start deterministic, add AI later), the full experience-ladder/skill-verification-on-completion system from the concept doc, and a `required_skills`-as-hard-filter gate (skills currently only affect the score, they don't hide non-matching postings — the UI still shows required-vs-preferred as separate labelled checks per listing, mirroring the doc's example).

**Not yet done**: nothing has been committed or pushed from this pivot — same established pattern as every other session, wait for an explicit go-ahead before `git commit`/`git push`.

## Renamed Shortlisted → Hitch, wordmark only (2026-08-17)

User picked "Hitch" as the new name (movie reference — a matchmaker, same concept applied to med students finding opportunities — plus the literal "hitching" tie-in to the marketplace pivot above). A new logomark was explored first (see chat history: Reve-generated blue "H" square read too close to Hinge's own icon, since Hinge also starts with H; pushed through several non-letterform concepts — tow hook, chain link, two joining circles, closed loop, hook-catching-a-ball — prototyped live in the actual nav bar and `src/app/icon.svg` favicon slot, screenshotted at both scales, then **reverted** because the icon direction wasn't settled). User's call: ship the name change now, defer the icon.

**What changed**: every user-facing "Shortlisted" string → "Hitch" — `layout.tsx` TITLE (drives page title + OG + Twitter card), `NavBar.tsx` and `Footer.tsx` wordmarks, `about/page.tsx` (heading + both body paragraphs + founder bio placeholders), `sources/page.tsx`, `contact/actions.ts` email `from` display name, plus the `shortlisted.app` placeholder domain in the `metadataBase` TODO comment.

**Deliberately left alone** — these all contain "Shortlisted"/"shortlisted" but aren't the brand name: `Toast.tsx`'s `ShortlistedToast` TS type (internal identifier, zero user visibility), `demoMode.ts`'s `shortlisted_demo_mode` localStorage key (internal, renaming would just orphan existing testers' flags for no benefit), `database.types.ts`'s header comment (accurately names the real Supabase project, which is still called "Shortlisted" at the infra level — not renamed), `cardiothoracic-portfolio-categories.ts`'s "Shortlisted or finalist, not winner" scoring label (the generic English word, unrelated), and `MarketplaceBrowser.tsx`/`MyPostings.tsx`'s "shortlisted" application-status string (same — generic word, real DB enum value).

**Still using the old logomark** (`/brand/logomark-progress-ring-transparent.png`, a progress-ring mark unrelated to the H-letterform/Hinge concern) — nav and favicon icon were never part of this pass. Next icon decision should build on the hook-concept exploration in the chat history rather than starting over: F2 (hook catching the ball) and B2 (closed loop) were the two finalists, prototyped and screenshotted, but neither was picked — F2 was rejected for reading like a fishhook catching bait rather than an actual trailer-hitch coupling.

**Not yet updated for the rename**: `about/page.tsx`'s body copy still frames the product around portfolio-building/self-assessment matrices (pre-pivot framing) — the user asked only for the name swap this pass, not a copy rewrite; worth revisiting alongside the rest of the marketplace-pivot copy above.
